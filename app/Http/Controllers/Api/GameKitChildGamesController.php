...
class GameKitChildGamesController extends Controller
{
    ...
    public function generateHangman(Request $request)
    {
        ...
        try { return response()->json(['words' => $this->ai->generateHangmanWords($data)]); }
    ...
    public function destroyHangman(Request $request, int $id) { $this->ownedHangman($request, $id)->delete(); return response()->json(['status' => 'deleted']); }
    ...
    private function cleanWords(array $words): array { return collect($words)->map(fn ($word) => mb_strtolower(trim(preg_replace('/[^\p{L} ]/u', '', (string) $word))))->filter(fn ($word) => mb_strlen($word) >= 3)->unique()->values()->all(); }
    private function ownedHangman(Request $request, int $id): GameKitHangmanGame { return GameKitHangmanGame::where('psychologist_id', $this->psychologistId($request))->findOrFail($id); }
    private function publicHangman(string $token): GameKitHangmanSession { $session = GameKitHangmanSession::with('game')->where('public_token_hash', hash('sha256', $token))->where('status', 'active')->where('public_token_expires_at', '>', now())->first(); abort_if(!$session, 404, 'Esta atividade não está mais disponível.'); return $session; }
    private function psychologistId(Request $request): int { $id = $request->user()->loadMissing('psychologist')->psychologist?->id; abort_if(!$id, 403, 'Perfil de psicólogo não encontrado.'); return (int) $id; }
}
