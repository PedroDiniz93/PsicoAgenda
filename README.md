...
## Execução do scheduler (Cron)

Para que as confirmações automáticas via WhatsApp sejam disparadas todos os dias, configure o cron do servidor para executar o scheduler do Laravel:
...
Configure o endpoint `POST {{APP_URL}}/api/webhook/whatsapp` no painel do WhatsApp Cloud API. Na etapa de verificação informe o `WHATSAPP_VERIFY_TOKEN` definido no `.env`. O webhook hoje:
